...
import PharmacistFormForm from "./pharmacist-form-form.vue";
import pharmacistFormStore from "./pharmacist-form-store";
...
export default {
  ...
  setup() {
    const data = reactive({
      ...
      selectedPharmacistIndex: 0,
    });
    const toast = inject("toast");
    const { t, locale } = useI18n({ useScope: "global" });
    provide("pharmacist_store", pharmacistFormStore);
    created();
    //Methods
    function onAddClicked() {
      data.selectedPharmacist = null;
      //Make little delay to ensure that watcher that found in category form component
      // catch selectedPharmacist input prop
      setTimeout(() => {
        pharmacistFormStore.onFormShow = !pharmacistFormStore.onFormShow;
      }, 1);
    }
    function onEditClicked(pharmacistForm, index) {
      data.selectedPharmacist = pharmacistForm;
      data.selectedPharmacistIndex = index;
      //Make little delay to ensure that watcher catch selectedPharmacist input prop
      //in category form component
      setTimeout(() => {
        pharmacistFormStore.onFormShow = !pharmacistFormStore.onFormShow;
      }, 1);
    }
    function onDeleteClicked(pharmacistForm, index) {
      data.selectedPharmacist = pharmacistForm;
      data.selectedPharmacistIndex = index;
    }
    function getPharmacistForms() {
      pharmacistFormClient
        .getPharmacistForms(data.page, data.pageSize, data.text)
        .then((response) => {
          data.pharmacistForms = response.data.data;
    ...
    function downloadExcelFile() {
      pharmacistFormClient.getPharmacistForms().then((res) => {
        let data = res.data;
        const fileName = "pharmacst-forms";
